...
const TrackRecordDetails = () => {
  ...
  const formatTime = (seconds: number, milliseconds: number) => {
    ...
    return `${minutes}:${secs < 10 ? "0" + secs : secs}.${Math.floor(
      milliseconds / 100
    )}`;
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.detailText}>
        Max Speed: {parseFloat(record.maxSpeed).toFixed(1)} Km/h
      </Text>
      <Text style={styles.detailText}>
        Avg Speed: {parseFloat(record.averageSpeed).toFixed(1)} Km/h
      </Text>
      <Text style={styles.detailText}>
        Total Time: {record.totalTime}.{record.totalMilliseconds} s
      </Text>
